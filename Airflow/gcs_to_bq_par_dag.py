...
with DAG(
    dag_id="gcs_to_bq_parquet_dag",
    description='gcs to Big Query Loading dag - For Parquet files',
    schedule='*/10 * * * *',
    max_active_runs=2,
    ...
    prefix="incoming/parq_data_",  # matches data_20260207.csv etc
    poke_interval=60,
    timeout=600,
    mode="reschedule",
    )
    ...
    fetch_data = GCSToBigQueryOperator(
        task_id="fetch_data",
        bucket="airflow_data_gcs",
        source_objects=["incoming/parq_data_*.parquet"],  # ✅ wildcard here
        destination_project_dataset_table="buoyant-striker-376011.staging_dataset.cust_table_parquet",
    ...
    archive_files = GCSToGCSOperator(
        ...
        source_object="incoming/parq_data_*.parquet",   # same pattern you loaded
        destination_bucket="airflow_data_gcs",
        destination_object="archive/parquet/",         # folder where files go
