...
with DAG(
    dag_id="gcs_to_bq_dag_with_dq",
    description='gcs to Big Query Loading dag With Data Quality Check',
    schedule='*/10 * * * *',
    max_active_runs=2,
    ...
    # prefix="incoming/",  # matches data_20260207.csv etc
    poke_interval=60,
    timeout=600,
    mode="reschedule",
    )
    ...
    fetch_data = GCSToBigQueryOperator(
        ...
        destination_project_dataset_table="buoyant-striker-376011.staging_dataset.cust_table_new",
        source_objects=["incoming/data_*.csv"],  
    ...
    dq_find_duplicates = BigQueryCheckOperator(
    task_id="dq_find_duplicate_ids",
    sql="""
    SELECT COUNT(*) = 0
    FROM (
      SELECT ID
      FROM `buoyant-striker-376011.staging_dataset.cust_table_new`
      GROUP BY ID
      HAVING COUNT(*) > 1
    )
    """,
...
# )
    ...
    start_task >> wait_for_file >> fetch_data >> dq_find_duplicates >> archive_files >> end_task